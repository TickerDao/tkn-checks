...
describe('Viem.js Mainnet Contract Interaction', () => {
  let contractAddress = '0xb097f4aA9F090037ea201B4810C526dD729b7240';
  let publicClient;
  ...
  test('Resolve Frame Token from base', async () => {
    const tick = 'FRAME';
    const data = await publicClient.readContract({  
      address: contractAddress,
      abi: tnsABI,
      functionName: 'lookup',
      args: [tick.toLowerCase()]
    })
    
    let fields = data.reduce((obj, { k, v }) => ({ ...obj, [k]: v }), {});
    expect(fields).toBeDefined();
    expect(fields).toHaveProperty('url', 'https://frametoken.us');
